...
function loadInvites()
{
	...
	for (var i=0; i<inviteJSON.length; i++)
	{
		...
		var orgName = Ti.UI.createLabel({text: inviteJSON[i]["org_name"], top: 20, left: 80, font: { fontSize:12, fontWeight: 'bold' }});
		row.add(orgName);
		var roleName = Ti.UI.createLabel({text: inviteJSON[i]["role"] + " invite", top: 40, left: 80, font: { fontSize:10}});
		row.add(roleName);
		var assignmentName = Ti.UI.createLabel({text: inviteJSON[i]["assignment_name"], top: 60, left: 80, font: { fontSize:10 }});
